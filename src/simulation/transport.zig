...
fn laneKey(v: Vehicle, road: usize) usize {
    return road * 4 + (if (v.node == city.roads[road].a) @as(usize, 0) else 2) + (if (v.line >= 0 and lanes[road] == 1) @as(usize, 1) else 0);
}
fn room(v: Vehicle, next: usize) bool {
    const road: usize = @intCast(city.road_between[v.node][next]);
    var candidate = v;
    candidate.next = next;
    var link = heads[laneKey(candidate, road)];
    while (link >= 0) {
        const other = vehicles[@intCast(link)];
        if (other.progress < (if (other.line >= 0) @as(f32, 3.2) else 2.0)) return false;
...
pub fn update(dt: f32, elapsed: f64) void {
    ...
    for (&vehicles, 0..) |*v, i| {
        ...
        if (v.node == v.next) {
            ...
            const r: usize = @intCast(city.road_between[v.node][next]);
            if (!city.roads[r].vehicles) continue;
            // Staggered two-phase signals and downstream space prevent entering a blocked link.
            const horizontal = city.nodes[next].z == city.nodes[v.node].z;
            const signal = @mod(elapsed + @as(f64, @floatFromInt(v.node % 3)), 12);
            if ((horizontal and signal >= 5) or (!horizontal and (signal < 6 or signal >= 11))) continue;
            if (!room(v.*, next)) continue;
            v.next = next;
            v.progress = 0;
            const key = laneKey(v.*, r);
            links[i] = heads[key];
        ...
        while (link >= 0) {
            ...
            if (@as(usize, @intCast(link)) != i and other.active and other.node == v.node and other.next == v.next and other.progress >= v.progress)
                free = @min(free, @max(0, other.progress - v.progress - (if (other.line >= 0) @as(f32, 3.2) else 2.0)));
            link = links[@intCast(link)];
        }
        ...
        const lane: f32 = if (v.line >= 0 and lanes[r] == 1) 1.25 else 0.48;
        ...
        if (v.progress >= road.length - 0.01) {
            v.node = v.next;
            v.speed = 0;
            if (v.line >= 0 and v.node == v.target) v.dwell = 5;
        }
    }
}
